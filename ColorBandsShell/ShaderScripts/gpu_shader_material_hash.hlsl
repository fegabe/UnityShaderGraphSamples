#ifndef __MATERIAL_HASH_HLSL__
#define __MATERIAL_HASH_HLSL__

/* https://github.com/gsage/engine/blob/86b72d180e67196841f874cf17e998e5efa3a1b0/resources/ogre/hlms/Common/HLSL/CrossPlatformSettings_piece_all.hlsl */
...
{
	return float2(hash_vec2_to_float(k), hash_vec3_to_float(float3(k, 1.0)));
}

float3 hash_vec3_to_vec3(float3 k)
{
	return float3(
		hash_vec3_to_float(k), hash_vec4_to_float(float4(k, 1.0)), hash_vec4_to_float(float4(k, 2.0)));
}
...
{
	return float3(
		hash_float_to_float(k), hash_vec2_to_float(float2(k, 1.0)), hash_vec2_to_float(float2(k, 2.0)));
}

float3 hash_vec2_to_vec3(float2 k)
{
	return float3(
		hash_vec2_to_float(k), hash_vec3_to_float(float3(k, 1.0)), hash_vec3_to_float(float3(k, 2.0)));
}

float3 hash_vec4_to_vec3(float4 k)
{
	return float3(hash_vec4_to_float(k.xyzw), hash_vec4_to_float(k.zxwy), hash_vec4_to_float(k.wzyx));
}
